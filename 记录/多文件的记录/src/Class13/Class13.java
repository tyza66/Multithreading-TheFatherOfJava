...
public class Class13 {
    /*
        ...
        如果要展示技术
        就可以把锁的升级过程展示出来
        偏向锁-》轻量级锁-》重量级锁  尽量不要使用到重量级锁，因为他的效率非常低
        轻量级锁底层用的就是CAS
        ...
        使用Lock锁 JUC并发包中的 需要自己实现锁的升级过程 底层是基于AQS+CAS实现的 依赖操作系统中的互斥指令 不属于重量级锁
        Threadlocal 想到与在每个线程中缓存局部变量 每个线程中就没有共享了 咩有了共享就没有了冲突的问题了 需要注意内存泄漏的问题
        还有个办法就是可以基于CAS自己手写
        工作三五年这样答是非常不常见的 要拿高新就必须谈的非常深
    */
}
